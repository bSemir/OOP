...
class Ucenik {
    static const int BrojPredmeta = 10; //prilikom testiranja smanjiti broj
...
class Razred {
    ...
    Ucenik **ucenici;
    static bool BoljiProsjek(const Ucenik *u1, const Ucenik *u2) { return u1->DajProsjek() > u2->DajProsjek(); }
public:
...
void Ucenik::Ispisi() const {
    ...
    if(DaLiJeProsao())
        std::cout << " ima prosjek " << std::setprecision(3) << DajProsjek() << std::endl;
    else
